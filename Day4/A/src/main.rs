...
fn get_score(board: Vec<Vec<i16>>, chosen: Vec<i16>)
{
    ...
    println!("Your score is: {}", score * last as i64);
}

fn solve(lines: Vec<String>) 
{
    let chosen = get_chosen(lines.clone());
    let boards = get_boards(lines.clone());
    ...
    for c in chosen
    {
        ...
        for board in &boards
        {
            if check_winner(board.clone(), curr_chosen.clone())
            {
                get_score(board.clone(), curr_chosen);
                exit(0);
            }
        }
    }
